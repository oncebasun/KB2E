...
class Train{
  ...
  public:
    ...
    /**
     * [run description]
     * @param n_in      [description]
     * @param rate_in   [description]
     * @param margin_in [description]
     * @param method_in [description]
     */
...
void prepare() {
  ...
  for (int i=0; i<relation_num; i++) {
  	...
  	left_num[i]=sum2/sum1;//每个关系的每个左节点平均对应多少个右节点
  }
  for (int i=0; i<relation_num; i++) {
  	double sum1=0,sum2=0;
  	...
  	right_num[i]=sum2/sum1;//每个关系的每个右节点平均对应多少个左节点
  }
