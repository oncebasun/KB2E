...
public:
  ...
  void run() {
    ...
    for (int testid = 0; testid<fb_l.size(); testid+=1) {
			...
			for (int i=0; i<entity_num; i++) {
				double sum = calc_sum(i,l,rel); // 掐头
				a.push_back(make_pair(i,sum));
			}
			sort(a.begin(),a.end(),cmp);
			double ttt=0;
			int filter = 0;
			for (int i=a.size()-1; i>=0; i--) { // 从排名高的往下捋
				if (ok[make_pair(a[i].first,rel)].count(l)>0) // 事实上有这个关系（但是可能不在测试集里）
					ttt++;
		    if (ok[make_pair(a[i].first,rel)].count(l)==0) // 事实上没这个关系
		    	filter+=1;
				if (a[i].first == h) { //正确答案
					lsum+=a.size()-i;
			...
			a.clear();
			for (int i=0; i<entity_num; i++) {
				double sum = calc_sum(h,i,rel); // 去尾
				a.push_back(make_pair(i,sum));
			}
			sort(a.begin(),a.end(),cmp);
			ttt=0;
